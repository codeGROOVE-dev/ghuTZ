...
func main() {
	...
	// Set up logger - only output logs if verbose flag is set
	...
	// Use gh auth token if no token specified
	token := *githubToken
	if token == "" {
	...
	if err != nil {
		cancel() // Call cancel before fatal
	...
	// Enhanced output with Apple-like polish
	fmt.Println()
	fmt.Printf("╭─────────────────────────────────────────────────────╮\n")
	...
	// Timezone with confidence
	tzConfidence := result.TimezoneConfidence
	if tzConfidence == 0 {
		tzConfidence = result.Confidence // Fallback for compatibility
	}
	...
	if result.GeminiSuggestedLocation != "" {
		// Prioritize Gemini's suggested location
		...
		// Try to derive location name from timezone if not set
		displayedLocation = getLocationNameForTimezone(result.Timezone)
		fmt.Printf("  📍 Location:   %s (approximate)\n", displayedLocation)
	} else {
		// Even without coordinates, provide educated guess from timezone and method
	...
	if result.Location != nil {
		...
		if locConfidence == 0 {
			locConfidence = result.Confidence * 0.8 // Slightly lower for location
		}
		...
		// Provide Google Maps search link even for approximate locations
...
<body>
    <div class="container">
        <h1>🌍 GitHub User Timezone Detector</h1>
